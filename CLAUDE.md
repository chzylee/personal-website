...
- **Self-animating constellation backdrop (hero):** behind the hero sits a canvas dot grid (`Hero.astro` `.hero__lattice` + `<script>`) that is *alive at rest* (it was previously near-invisible until the cursor moved, which read as "just plain black"; this was changed deliberately). The field is lit by **three soft "energy centers"**: the cursor (the primary, strongest center, radius ~220px on desktop) plus **two ambient centers that slowly WANDER to random targets** (re-picking a new target on arrival), independent of the cursor (~176px radius, dimmer via a per-center `glow` weight). The wander gives the deliberate feel of "something alive moving around slowly," vs. a predictable looping path. Roamer positions are stored as fractions of the canvas so they survive a resize. Within any center's reach, nodes gather (magnetic pull, contributions from all centers accumulate and the lit factor is clamped to 1), brighten toward `--color-rust`, and "wire up" with low-alpha links between lit neighbours — so 2-3 pools of activity are always drifting across the grid and links form/dissolve in waves. Every node also carries a faint per-node **twinkle** (golden-angle-scattered `phase`, slow global sine) so the dark field between pools shimmers instead of reading as flat black. A soft purple `.hero__bloom` (radial gradient via `color-mix` on `--color-rust`) tracks the eased cursor center for depth; when the pointer is idle (or on touch / no-pointer devices) that center slowly auto-roams. The canvas reads brand colors from the CSS tokens at runtime (no hardcoded hex). Performance: `devicePixelRatio` capped at 2, typed-array node store (incl. `phase`) allocated on resize (not per frame), inner loop is O(nodes × 3 centers), rAF paused via `IntersectionObserver` when off-screen and via `visibilitychange` when the tab is hidden. Animates only canvas draws + a compositor `transform` on the bloom. Honors `prefers-reduced-motion`: renders a single static frame (centers/twinkle frozen at t=0), no loop. This is the one allowed exception to "No `overflow: hidden` as a lazy fix" (the hero clips the bloom on purpose). History: an even earlier version made the lattice the loud full-bleed focal element with a hidden FGC-input mini-game; that read as cluttered ("over everything"), so it was demoted to texture and the storytelling moved to the CRT screen (below). The current constellation is the middle ground: always-on life without being the focal point — the CRT remains the right-side "wow". Alternative hero directions are catalogued in the gitignored `HERO-DIRECTIONS.local.md`.
- **TV backdrop / event reel (hero):** the hero has a **full-bleed muted video backdrop** (`Hero.astro` `.hero__tv`) that the visitor powers on via a `.hero__tv-toggle` button ("Roll the event reel" / "Turn off the TV"). On power-on, the constellation fades out (its rAF stops, gated by a `tvOn` flag so the `IntersectionObserver`/`visibilitychange` handlers don't restart it), and a muted/looping/`playsinline` event-reel plays behind the content with a **CRT power-on flash + collapse-line reveal** (`@keyframes tv-power-on` on `.hero__tv-crt`, `tv-flash` on `.hero__tv-flash`), plus `scanlines` + `vignette` overlays and a left/bottom **legibility scrim** so the hero text stays readable over video. The reel is `preload="none"` and lazy-`load()`ed on first power-on, and **pauses when the hero scrolls off-screen or the tab is hidden** (its own `IntersectionObserver` + `visibilitychange`). Until a real reel is added at `public/video/event-reel.{webm,mp4}`, a **"NO SIGNAL" CRT test card** (`.hero__tv-nosignal`) shows; the `<video>`'s `playing` event adds `.is-live` (reveal video, hide test card), and an `error` keeps the test card, so dropping the files in "just works" with no code change. The scanline `repeating-linear-gradient` is an intentional CRT effect on `pointer-events: none` overlays, NOT the banned decorative-stripe pattern (same justification as the CRT screen below). Honors `prefers-reduced-motion`: the power-on flash/collapse animation is disabled (the reel still fades in). All colors via `color-mix` on tokens; the flash uses `--color-ink` (warm off-white), never `#fff`. Justified: the user wanted a thematic "turn on the TV" moment that sets a living backdrop and doubles as FGC/community proof (the event reel), without sound. Provide a ~20-40s muted 720p mashup; YouTube embed was rejected for a background loop (iframe chrome / branding / no seamless silent loop).
